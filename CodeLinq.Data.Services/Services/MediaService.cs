...
{
    /// <summary>
    /// A Service that contains methods to perform operations on IMedia
    /// </summary>
    public class MediaService : IMediaService
    {
        ...
        public MediaService(IRepository<IMedia> mediaRepository, IFileSystemProvider fileSystemProvider)
        {
        ...
        public IOperationResult<IMedia> Delete(object mediaId)
        {
            return Delete(mediaRepository.Get(mediaId));
        }
        ...
        /// <returns>An IOperationResult<IMedia> object containing the results of the operation</returns>
        public IOperationResult<IMedia> Delete(IMedia media)
        {
            try
            {
                // check if file exists on physical storage
                if (!fileSystemProvider.FileExits(media.FilePath))
                {
                    return new OperationResult<IMedia>
                    {
                        OperationOutcome = OperationOutcome.NotFound,
                        ResultCode = -1,
                        Message = $"'{media.FilePath}' not found.",
                        Entity = media
                    };
                }

                // see if the filesystem provider can delete the physical file
                if (!fileSystemProvider.DeleteFile(media.FilePath))
                {
                    return new OperationResult<IMedia>
                    {
                        OperationOutcome = OperationOutcome.InternalError,
                        ResultCode = -1,
                        Message = $"Unable to delete file '{media.FilePath}'",
                        Entity = media
                    };
                }

                // try delete the IMedia record from the datastore
                return mediaRepository.Delete(media);
            }
            catch (System.Exception ex)
            {
                // some error occured, so return details about the exception
                return new OperationResult<IMedia>
                {
                    OperationOutcome = OperationOutcome.InternalError,
                    ResultCode = -1,
                    Message = $"Error deleting file '{media.FilePath}': {ex.Message}",
                    Entity = media
                };
            }
        }
        ...
        /// <returns>An IOperationResult<IMedia> object containing the results of the operation</returns>
        public IOperationResult<IMedia> Save(byte[] data, string uploadedFileName, object entityId, EntityType entityType, MediaType mediaType)
        {
            try
            {
                //Input validation
                ValidateSaveInputParams(data, uploadedFileName, entityId);

                // save using filesystem provider
                var filePath = fileSystemProvider.StoreFile(data, uploadedFileName);
                ...
                media.Name = System.IO.Path.GetFileName(media.FilePath);

                //save to repository
                var repoResult = mediaRepository.Insert(media);
                //media.Id = repoResult.Entity.Id;
                media = repoResult.Entity;
                ...
                return new OperationResult<IMedia>
                {
                    ...
                    Message = $"Media saved succesfully to {media.FilePath}"
                };
            }
            catch (Exception ex)
            {
        ...
        private void ValidateSaveInputParams(byte[] data, string uploadedFileName, object entityId)
        {
